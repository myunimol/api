...
public class UnimolLoginParser implements LoginParser {
	...
	private UserInformation parsingUserInfo(Document doc) {
		...
		// e' l'indicazione di una
		// informazione utile allora vuol dire che quello attuale e'
		// l'informazione da recuperare
		String precedente = "";
		for (Element tableCell : addInform) {
			if (precedente.equals("Tasse")) { // situazione tasse es: regolare
				uInfo.setTaxes(tableCell.text());
			} else if (precedente.equals("Piano carriera")) { // se il piano
																// carriera e'
																// modificabile
				uInfo.setCareerPlan(tableCell.text());
			} else if (precedente.equals("Appelli disponibili")) { // quanti
																	// appelli
																	// disponibili
																	// ci sono
				uInfo.setAvailableExams(tableCell.text());
			} else if (precedente.equals("Iscrizioni appelli")) { // a quanti
																	// appelli
																	// lo
																	// studente
																	// e'
	...
	/**
	 * Recupera la matricola da un'altra pagina! dato che non &egrave; presente
	 * nella home di esse3
	 *
	 * @return lo studentID
	 */
	private String getStudentID(String username, String password)
		...
		/**
		 * dato che la stringa ha questa forma:
		 * "Libretto di : IVAN DI RIENZO - [MAT. 148432]" elimina la parentesi
		 * chiusa e infine splitta dividendo da "MAT. " e prende la seconda
		 * stringa ottenuta cioe' la matricola
		 */
